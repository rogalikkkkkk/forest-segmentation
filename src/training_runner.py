...
def train_one_epoch(
    ...
    for batch_index, batch in enumerate(dataloader, start=1):
        ...
        if log_every_n_batches > 0 and batch_index % log_every_n_batches == 0:
            print(
...
def parse_training_args(model_spec: ModelSpec):
    parser = argparse.ArgumentParser(
        description=f"Train {model_spec.display_name} on RUGD.",
    )
    ...
    parser.add_argument("--loss", choices=SUPPORTED_LOSSES, default=model_spec.loss)
...
def should_save_epoch_predictions(selected_indices, epoch, args, prediction_epochs):
    return selected_indices and (
...
def train_model(*, model_spec: ModelSpec, create_model, args):
    run_dir = (
        prepare_run_dir(model_spec.output_dir, run_dir=args.run_dir)
        if args.run_dir is not None
        else None
    )
    artifacts_dir = run_dir if run_dir is not None else model_spec.output_dir
    artifacts_dir.mkdir(parents=True, exist_ok=True)
    checkpoint_path = artifacts_dir / model_spec.checkpoint_path.name
    best_checkpoint_path = artifacts_dir / model_spec.best_checkpoint_path.name
    history_path = artifacts_dir / model_spec.history_path.name
    step_history_path = artifacts_dir / model_spec.step_history_path.name
    best_metrics_path = artifacts_dir / model_spec.best_metrics_path.name
    training_curves_path = artifacts_dir / model_spec.training_curves_path.name
    step_loss_curve_path = artifacts_dir / model_spec.step_loss_curve_path.name
    ...
    print(f"Training {model_spec.display_name}")
    print("=" * 60)
    ...
    print(f"Encoder weights: {model_spec.encoder_weights}")
    ...
    print(f"Batch logging interval: {model_spec.log_every_n_batches}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        train_loss, global_step = train_one_epoch(
            ...
            log_every_n_batches=model_spec.log_every_n_batches,
        )
        ...
        save_predictions = should_save_epoch_predictions(
            selected_val_indices,
            epoch,
            args,
            prediction_epochs,
        ...
        if save_predictions:
            ...
            save_epoch_prediction_grid(
                ...
                image_mean=IMAGE_MEAN,
                image_std=IMAGE_STD,
                title=f"{model_spec.display_name} - epoch {epoch}",
            )
...
def run_training_cli(model_spec: ModelSpec, create_model):
    args = parse_training_args(model_spec)
    train_model(model_spec=model_spec, create_model=create_model, args=args)
