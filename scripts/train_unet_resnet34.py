...
def save_history(history):
    ...
    with UNET_RESNET34_HISTORY_PATH.open("w", encoding="utf-8", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=fieldnames)
        writer.writeheader()
        writer.writerows(history)

    with UNET_RESNET34_LOSS_HISTORY_PATH.open("w", encoding="utf-8") as file:
        for row in history:
...
def save_step_history(step_history):
    ...
    with UNET_RESNET34_STEP_HISTORY_PATH.open(
        "w",
        encoding="utf-8",
        newline="",
    ) as file:
...
def save_best_metrics_with_class_statistics(
    ...
    with UNET_RESNET34_BEST_METRICS_PATH.open("w", encoding="utf-8") as file:
...
def plot_training_curves(history):
    ...
    plt.tight_layout()
    plt.savefig(UNET_RESNET34_TRAINING_CURVES_PATH, dpi=150)
    plt.close()
...
def plot_step_loss_curve(step_history):
    ...
    plt.tight_layout()
    plt.savefig(UNET_RESNET34_STEP_LOSS_CURVE_PATH, dpi=150)
    plt.close()
...
def main():
    ...
    epoch_predictions_dir = (
        (run_dir if run_dir is not None else UNET_RESNET34_OUTPUT_DIR)
        / "epoch_predictions"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(checkpoint, UNET_RESNET34_CHECKPOINT_PATH)

        if is_best:
            torch.save(checkpoint, UNET_RESNET34_BEST_CHECKPOINT_PATH)
        ...
        if is_best:
            print(f"Best checkpoint updated: {UNET_RESNET34_BEST_CHECKPOINT_PATH}")
        if (
            selected_val_indices
            and (
                epoch in prediction_epochs
    ...
    print(f"Last checkpoint saved to: {UNET_RESNET34_CHECKPOINT_PATH}")
    print(f"Best checkpoint saved to: {UNET_RESNET34_BEST_CHECKPOINT_PATH}")
    print(f"History saved to: {UNET_RESNET34_HISTORY_PATH}")
    print(f"Step history saved to: {UNET_RESNET34_STEP_HISTORY_PATH}")
    print(f"Loss history saved to: {UNET_RESNET34_LOSS_HISTORY_PATH}")
    print(f"Best metrics saved to: {UNET_RESNET34_BEST_METRICS_PATH}")
    print(f"Training curves saved to: {UNET_RESNET34_TRAINING_CURVES_PATH}")
    print(f"Step loss curve saved to: {UNET_RESNET34_STEP_LOSS_CURVE_PATH}")
    if selected_val_indices:
