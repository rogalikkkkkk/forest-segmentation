...
from config import (
    ...
    DEEPLABV3PLUS_RESNET50_VAL_SPLIT_PATH,
...
def train_one_epoch(
    ...
    for batch_index, batch in enumerate(dataloader, start=1):
        ...
        if (
            DEEPLABV3PLUS_RESNET50_LOG_EVERY_N_BATCHES > 0
            and batch_index % DEEPLABV3PLUS_RESNET50_LOG_EVERY_N_BATCHES == 0
        ):
            print(
...
def create_checkpoint(
    ...
    return {
        ...
        "train_images_dir": str(DEEPLABV3PLUS_RESNET50_TRAIN_IMAGES_DIR),
        "train_masks_id_dir": str(DEEPLABV3PLUS_RESNET50_TRAIN_MASKS_ID_DIR),
        "train_split_path": str(DEEPLABV3PLUS_RESNET50_TRAIN_SPLIT_PATH),
        "val_split_path": str(DEEPLABV3PLUS_RESNET50_VAL_SPLIT_PATH),
    }
...
def save_history(history):
    ...
    with DEEPLABV3PLUS_RESNET50_HISTORY_PATH.open("w", encoding="utf-8", newline="") as file:
...
def save_step_history(step_history):
    ...
    with DEEPLABV3PLUS_RESNET50_STEP_HISTORY_PATH.open(
        "w",
        encoding="utf-8",
        newline="",
    ) as file:
...
def save_best_metrics_with_class_statistics(
    ...
    with DEEPLABV3PLUS_RESNET50_BEST_METRICS_PATH.open("w", encoding="utf-8") as file:
...
def plot_training_curves(history):
    ...
    plt.tight_layout()
    plt.savefig(DEEPLABV3PLUS_RESNET50_TRAINING_CURVES_PATH, dpi=150)
    plt.close()
...
def plot_step_loss_curve(step_history):
    ...
    plt.tight_layout()
    plt.savefig(DEEPLABV3PLUS_RESNET50_STEP_LOSS_CURVE_PATH, dpi=150)
    plt.close()


def parse_args():
    parser = argparse.ArgumentParser(description="Train DeepLabV3+ ResNet50 on RUGD.")
    ...
    parser.add_argument("--loss", choices=SUPPORTED_LOSSES, default=DEEPLABV3PLUS_RESNET50_LOSS)
...
def main():
    args = parse_args()
    run_dir = (
        prepare_run_dir(DEEPLABV3PLUS_RESNET50_OUTPUT_DIR, run_dir=args.run_dir)
    ...
    val_dataset = RUGDDataset(
        ...
        image_mean=IMAGE_MEAN,
        image_std=IMAGE_STD,
        split_file=DEEPLABV3PLUS_RESNET50_VAL_SPLIT_PATH,
    )
    ...
    epoch_predictions_dir = (
        (run_dir if run_dir is not None else DEEPLABV3PLUS_RESNET50_OUTPUT_DIR)
        / "epoch_predictions"
    )
    ...
    print(f"Val split: {DEEPLABV3PLUS_RESNET50_VAL_SPLIT_PATH}")
    print(f"Train size: {len(train_dataset)}")
    print(f"Val size: {len(val_dataset)}")
    print(f"Image size: {IMAGE_HEIGHT} x {IMAGE_WIDTH}")
    print(f"Batch size: {BATCH_SIZE}")
    print(f"Number of classes: {RUGD_NUM_CLASSES}")
    print(f"Encoder weights: {DEEPLABV3PLUS_RESNET50_ENCODER_WEIGHTS}")
    ...
    print(f"Batch logging interval: {DEEPLABV3PLUS_RESNET50_LOG_EVERY_N_BATCHES}")
    ...
    save_config_snapshot(
        run_dir,
        {
            ...
            "train_split": DEEPLABV3PLUS_RESNET50_TRAIN_SPLIT_PATH,
            "val_split": DEEPLABV3PLUS_RESNET50_VAL_SPLIT_PATH,
        },
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(checkpoint, DEEPLABV3PLUS_RESNET50_CHECKPOINT_PATH)

        if is_best:
            torch.save(checkpoint, DEEPLABV3PLUS_RESNET50_BEST_CHECKPOINT_PATH)
        ...
        if (
            ...
            save_epoch_prediction_grid(
                ...
                image_mean=IMAGE_MEAN,
                image_std=IMAGE_STD,
                title=f"DeepLabV3+ ResNet50 - epoch {epoch}",
            )
        ...
        if is_best:
            print(f"Best checkpoint updated: {DEEPLABV3PLUS_RESNET50_BEST_CHECKPOINT_PATH}")
        if (
            selected_val_indices
            and (
                epoch in prediction_epochs
    ...
    print(f"Last checkpoint saved to: {DEEPLABV3PLUS_RESNET50_CHECKPOINT_PATH}")
